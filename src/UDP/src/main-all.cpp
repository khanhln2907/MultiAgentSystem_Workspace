...
//************************* Controller Here ******************************
  if ((cvRound(p[2])>minradius) && circles->total )

  { //ROS_INFO("X = %d, Y= %d, R= %d", cvRound(p[0]), cvRound(p[1]), cvRound(p[2]));
    ctrlinput[3] = 700 + 0.5 * (cvRound(p[1])- 120);     // thrust link to y coordinates
    ctrlinput[2] = 0 - 0.2 * (160 - cvRound(p[0]));        // yaw link to x coordinates
    ctrlinput[0] = 5 + 0.2 * (maxradius - 20 - cvRound(p[2]));      // pitch link to radius
  }
...
//              cout<<"Բ�����x= "<<centerx<<endl<<"Բ�����y= "<<centery<<endl;
//              cout<<"�뾶="<<radius<<endl;
//           }
...
ctrlinput[1]=0;     //roll
ctrlinput[2]=20;     //yaw
ctrlinput[3]=700;   //throttle & thrust
ctrlinput[4]=0x0F;
...
//      ROS_INFO("Thrust= %d, Pitch= %d, Roll= %d, Yaw= %d", ctrlinput[3],ctrlinput[0],ctrlinput[1],ctrlinput[2]);
      ros::spinOnce();
      r.sleep();
    } 
