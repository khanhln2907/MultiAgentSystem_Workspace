...
geometry_msgs::Vector3 center_position;
geometry_msgs::Vector3 center_orientation;
...
double *findIntersection1(double line1[], double line2[], int x_limit_min, int x_limit_max, int y_limit_min, int y_limit_max)
{
    double x1,x2,y1,y2,x3,y3,x4,y4,denominator,xNominator,yNominator,px,py;
    static double intersection[2];
...
{
    center_position.x = center.position.x;
    center_position.y = center.position.y;
    center_orientation.x = center.orientation.z;
    center_orientation.y = center.orientation.w;
//	ROS_INFO("I'm in!\n");
...
//    ROS_INFO("I'm in again!\n");
    double alpha;
 	   
    alpha = 2*atan(center_orientation.x/center_orientation.y);
    ...
    z_x = center_position.x*1000 - 162/0.5416*sin(alpha+1.5707963);
    z_y = center_position.y*1000 + 162/0.5416*cos(alpha+1.5707963);
    v_0 = 16;
    omega_0 = 0.5416;
...
/********************************************************************************switching control*****************************************************************************/
    ...
    if (k == 50)
    {
	v = sqrt((center_position.x - temp_x)*(center_position.x - temp_x)*1000000 + (center_position.y - temp_y)*(center_position.y - temp_y)*1000000)/0.5;
	temp_x = center_position.x;
    	temp_y = center_position.y;
	//printf("speed=%f\n", v);
	k = 0;
    }	 
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pub = push.advertise<geometry_msgs::Vector3>("/vehicle/vt_ctr",100);
    ros::Publisher pubc = cent.advertise<geometry_msgs::Vector3>("/vehicle/centroid",100);   
    ros::Rate rate(10);
    ...
    center_position.y = 0;
    center_position.z = 0;
    center_orientation.x = 0;
    center_orientation.y = 0;
    center_orientation.z = 0;

    ros::Subscriber sub = pull.subscribe("/qualisys/Center", 100, Translate);
    ros::Subscriber neib1 = neighbour1.subscribe("/jet1/vt_ctr", 100, N1call);
    ros::Subscriber neib2 = neighbour2.subscribe("/jet2/vt_ctr", 100, N2call);
