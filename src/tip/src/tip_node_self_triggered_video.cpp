...
geometry_msgs::Twist n1_ctr;      //previous msg type: Vector3
geometry_msgs::Twist n2_ctr;      //previous msg type: Vector3
geometry_msgs::Twist n3_ctr;      //previous msg type: Vector3
geometry_msgs::Twist vt_ctr;      //previous msg type: Vector3
...
Vector2 agent_0_dot_z;
Vector2 agent_1_dot_z;
Vector2 agent_2_dot_z;
Vector2 agent_3_dot_z;


void N1call(const geometry_msgs::Twist& n1)    //previous msg type: Vector3
{
    agent_1.x = (long)n1.linear.x;
    agent_1.y = (long)n1.linear.y;

    agent_1_dot_z.x = (long)n1.angular.x;
    agent_1_dot_z.y = (long)n1.angular.y;
}

void N2call(const geometry_msgs::Twist& n2)    //previous msg type: Vector3
{
    agent_2.x = (long)n2.linear.x;
    agent_2.y = (long)n2.linear.y;

    agent_2_dot_z.x = (long)n2.angular.x;
    agent_2_dot_z.y = (long)n2.angular.y;
}

void N3call(const geometry_msgs::Twist& n3)  //previous msg type: Vector3
{
    agent_3.x = (long)n3.linear.x;
    agent_3.y = (long)n3.linear.y;

    agent_3_dot_z.x = (long)n3.angular.x;
    agent_3_dot_z.y = (long)n3.angular.y;
}
...
long partial_derivative_centroid_over_self_state_x_x(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{

    //int t = 1;
    long boundary_length, distance, output_1, output_2, output;

    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (x_1*x_1 + x_1*(x_2 - x_1) - x_1*z_k_x + (x_2 - x_1)*(x_2 - x_1)/3 - z_k_x*(x_2 - x_1)/2 )*boundary_length/distance/mass;
    output_2 = (x_1 + (x_2 - x_1)/2 - z_k_x)*boundary_length/distance*c_x/mass; 
...
long partial_derivative_centroid_over_self_state_y_x(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{
    ...
    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (x_1*y_1 + y_1*(x_2 - x_1)/2 - y_1*z_k_x + x_1*(y_2 - y_1)/2 + (y_2 - y_1)*(x_2 - x_1)/3 - z_k_x*(y_2 - y_1)/2 )*boundary_length/distance/mass;
    output_2 = (x_1 + (x_2 - x_1)/2 - z_k_x)*boundary_length/distance*c_y/mass;
...
long partial_derivative_centroid_over_self_state_x_y(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{
    ...
    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (x_1*y_1 + x_1*(y_2 - y_1)/2 - x_1*z_k_y + y_1*(x_2 - x_1)/2 + (y_2 - y_1)*(x_2 - x_1)/3 - z_k_y*(x_2 - x_1)/2 )*boundary_length/distance/mass;
    output_2 = (y_1 + (y_2 - y_1)/2 - z_k_y)*boundary_length/distance*c_x/mass;
...
long partial_derivative_centroid_over_self_state_y_y(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{
    ...
    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (y_1*y_1 + y_1*(y_2 - y_1) - y_1*z_k_y + (y_2 - y_1)*(y_2 - y_1)/3 - z_k_y*(y_2 - y_1)/2 )*boundary_length/distance/mass;
    output_2 = (y_1 + (y_2 - y_1)/2 - z_k_y)*boundary_length/distance*c_y/mass;
...
long partial_derivative_centroid_over_neighbour_state_x_x(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{
    ...
    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (x_1*z_j_x - x_1*x_1 - x_1*(x_2 - x_1) + z_j_x*(x_2 - x_1)/2 - (x_2 - x_1)*(x_2 - x_1)/3)*boundary_length/distance/mass;
    output_2 = (z_j_x - x_1 - (x_2 - x_1)/2 )*boundary_length/distance*c_x/mass; 
...
long partial_derivative_centroid_over_neighbour_state_y_x(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{
    ...
    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (y_1*z_j_x - x_1*y_1 - y_1*(x_2 - x_1)/2 + z_j_x*(y_2 - y_1)/2 - x_1*(y_2 - y_1)/2 - (y_2 - y_1)*(x_2 - x_1)/3)*boundary_length/distance/mass;
    output_2 = (z_j_x - x_1 - (x_2 - x_1)/2 )*boundary_length/distance*c_y/mass;
...
long partial_derivative_centroid_over_neighbour_state_x_y(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{
    ...
    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (x_1*z_j_y - x_1*y_1 - x_1*(y_2 - y_1)/2 + (x_2 - x_1)*z_j_y/2 - y_1*(x_2 - x_1)/2 - (x_2 - x_1)*(y_2 - y_1)/3)*boundary_length/distance/mass;
    output_2 = (z_j_x - y_1 - (y_2 - y_1)/2 )*boundary_length/distance*c_x/mass;
...
long partial_derivative_centroid_over_neighbour_state_y_y(long x_1, long y_1, long x_2, long y_2, long z_k_x, long z_k_y, long z_j_x, long z_j_y, long mass, long c_x, long c_y)
{
    ...
    boundary_length = sqrt( (x_2 - x_1)*(x_2 - x_1) + (y_2 - y_1)*(y_2 - y_1));
    distance = sqrt( (z_j_x - z_k_x)*(z_j_x - z_k_x) + (z_j_y - z_k_y)*(z_j_y - z_k_y));
    output_1 = (y_1*z_j_y - y_1*y_1 - y_1*(y_2 - y_1)/2 + z_j_y*(y_2 - y_1)/2 - y_1*(y_2 - y_1)/2 - (y_2 - y_1)*(y_2 - y_1)/3)*boundary_length/distance/mass;
    output_2 = (z_j_x - y_1 - (y_2 - y_1)/2 )*boundary_length/distance*c_y/mass;
...
CentroidMass computeCentroid(long Vertices_X[], long Vertices_Y[], int nVertices)
{

    CentroidMass Centroid_P;
...
void voronoi_area_compute(Point agent_0, Point agent_1, Point agent_2, Point agent_3)
{
    ...
    Vector2 connect_agent_1_dot_z;
    Vector2 connect_agent_2_dot_z;
    Vector2 connect_agent_3_dot_z;	
    ...
    long boundary1_vertices[4] = {0};
    long boundary2_vertices[4] = {0};
    long boundary3_vertices[4] = {0};
	...
	connect_agent_1.y = agent_1.x;
        connect_agent_1_dot_z.x = agent_1_dot_z.x;
        connect_agent_1_dot_z.y = agent_1_dot_z.y;

	connect_agent_2.x = agent_2.x;
	connect_agent_2.y = agent_2.y;
        connect_agent_2_dot_z.x = agent_2_dot_z.x;
        connect_agent_2_dot_z.y = agent_2_dot_z.y;

	connect_agent_3.x = agent_3.x;
	connect_agent_3.y = agent_3.y;
        connect_agent_3_dot_z.x = agent_3_dot_z.x;
        connect_agent_3_dot_z.y = agent_3_dot_z.y;
	...
	connect_agent_2.y = agent_3.y;
        connect_agent_2_dot_z.x = agent_3_dot_z.x;
        connect_agent_2_dot_z.y = agent_3_dot_z.y;

	connect_agent_3.x = agent_2.x;
	connect_agent_3.y = agent_2.y;
        connect_agent_3_dot_z.x = agent_2_dot_z.x;
        connect_agent_3_dot_z.y = agent_2_dot_z.y;
	...
	connect_agent_1.y = agent_2.y;
        connect_agent_1_dot_z.x = agent_2_dot_z.x;
        connect_agent_1_dot_z.y = agent_2_dot_z.y;

	connect_agent_2.x = agent_1.x;
	connect_agent_2.y = agent_1.y;
        connect_agent_2_dot_z.x = agent_1_dot_z.x;
        connect_agent_2_dot_z.y = agent_1_dot_z.y;
	...
	connect_agent_3.y = agent_1.y;
        connect_agent_3_dot_z.x = agent_1_dot_z.x;
        connect_agent_3_dot_z.y = agent_1_dot_z.y;
	...
	connect_agent_1.y = agent_3.y;
        connect_agent_1_dot_z.x = agent_3_dot_z.x;
        connect_agent_1_dot_z.y = agent_3_dot_z.y;
	...
	*/
        ...
        if ( (max_radius_temp > distance_2) && (max_radius <= distance_3) )
	{
		...
		V1_vertice_counter = V1_vertice_counter + 1;
		    ...
		    boundary2_vertices[0] = V1_segment_2_intersection.x1;
		    boundary2_vertices[1] = V1_segment_2_intersection.y1;		
	    	    V1_vertice_counter = V1_vertice_counter + 1;
	    	}
		    ...
		    boundary2_vertices[0] = V1_segment_2_intersection.x2;
		    boundary2_vertices[1] = V1_segment_2_intersection.y2;
	    	    V1_vertice_counter = V1_vertice_counter + 1;
	    	}
    	    }    
		    ...
		    boundary1_vertices[0] = V1_segment_1_intersection.x1;
		    boundary1_vertices[1] = V1_segment_1_intersection.y1;
	    	    V1_vertice_counter = V1_vertice_counter + 1;
       	    	}
		    ...
		    boundary1_vertices[0] = V1_segment_1_intersection.x2;
		    boundary1_vertices[1] = V1_segment_1_intersection.y2;
	    	    V1_vertice_counter = V1_vertice_counter + 1;
       	    	}
    	    }	   
		...
		boundary1_vertices[2] = V1_s1_s2_inter_point.x;
		boundary1_vertices[3] = V1_s1_s2_inter_point.y;
		boundary2_vertices[2] = V1_s1_s2_inter_point.x;
		boundary2_vertices[3] = V1_s1_s2_inter_point.y;
		V1_vertice_counter = V1_vertice_counter + 1;
    	    }
    	}
	...
	else if ( max_radius > distance_3 )    /*compute voronoi area with three neighbours*/
	{
	    ...
	    */
		...
		boundary3_vertices[0] = V1_segment_3_intersection.x1;
		boundary3_vertices[1] = V1_segment_3_intersection.y1;
	    	V1_vertice_counter = V1_vertice_counter + 1;
    	    }
		...
		boundary3_vertices[0] = V1_segment_3_intersection.x2;
		boundary3_vertices[1] = V1_segment_3_intersection.y2;
	    	V1_vertice_counter = V1_vertice_counter + 1;
    	    }
		...
		boundary2_vertices[2] = V1_s2_s3_inter_point.x;
		boundary2_vertices[3] = V1_s2_s3_inter_point.y;
		boundary3_vertices[2] = V1_s2_s3_inter_point.x;
		boundary3_vertices[3] = V1_s2_s3_inter_point.y;
	    	V1_vertice_counter = V1_vertice_counter + 1;	    	
	    }
		...
		boundary1_vertices[2] = V1_s1_s3_inter_point.x;
		boundary1_vertices[3] = V1_s1_s3_inter_point.y;
		boundary3_vertices[2] = V1_s1_s3_inter_point.x;
		boundary3_vertices[3] = V1_s1_s3_inter_point.y;
	    	V1_vertice_counter = V1_vertice_counter + 1;	    	
	    }
	...
	//ROS_INFO("V1_vertice_counter: %d", V1_vertice_counter);   

	} 


        //ROS_INFO("v1_vertices: %ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld", V1_vertices[0], V1_vertices[1], V1_vertices[2], V1_vertices[3], V1_vertices[4], V1_vertices[5], V1_vertices[6], V1_vertices[7]);
	...
	//compute Jacobian of C_v_k;

	long p_1_s_x_x = 0, p_2_s_x_x = 0, p_3_s_x_x = 0, p_1_s_y_x = 0, p_2_s_y_x = 0, p_3_s_y_x = 0, p_1_s_x_y = 0, p_2_s_x_y = 0, p_3_s_x_y = 0, p_1_s_y_y = 0, p_2_s_y_y = 0, p_3_s_y_y = 0;
        long p_1_n_x_x = 0, p_2_n_x_x = 0, p_3_n_x_x = 0, p_1_n_y_x = 0, p_2_n_y_x = 0, p_3_n_y_x = 0, p_1_n_x_y = 0, p_2_n_x_y = 0, p_3_n_x_y = 0, p_1_n_y_y = 0, p_2_n_y_y = 0, p_3_n_y_y = 0;
	if (V1_vertice_counter == 4)   //The agent has two neighbours	
	{
	
	    p_1_s_x_x = partial_derivative_centroid_over_self_state_x_x(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_s_x_x = partial_derivative_centroid_over_self_state_x_x(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  

	    p_1_s_y_x = partial_derivative_centroid_over_self_state_y_x(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_s_y_x = partial_derivative_centroid_over_self_state_y_x(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  	

	    p_1_s_x_y = partial_derivative_centroid_over_self_state_x_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_s_x_y = partial_derivative_centroid_over_self_state_x_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_1_s_y_y = partial_derivative_centroid_over_self_state_y_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_s_y_y = partial_derivative_centroid_over_self_state_y_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y); 

	    p_1_n_x_x = partial_derivative_centroid_over_neighbour_state_x_x(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_x_x = partial_derivative_centroid_over_neighbour_state_x_x(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  

	    p_1_n_y_x = partial_derivative_centroid_over_neighbour_state_y_x(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_y_x = partial_derivative_centroid_over_neighbour_state_y_x(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  	

	    p_1_n_x_y = partial_derivative_centroid_over_neighbour_state_x_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_x_y = partial_derivative_centroid_over_neighbour_state_x_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_1_n_y_y = partial_derivative_centroid_over_neighbour_state_y_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_y_y = partial_derivative_centroid_over_neighbour_state_y_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  	  	

	    dot_c_v_x = (p_1_s_x_x + p_2_s_x_x)*agent_0_dot_z.x + (p_1_s_x_y + p_2_s_x_y)*agent_0_dot_z.y + p_1_n_x_x*connect_agent_1_dot_z.x + p_1_n_x_y*connect_agent_1_dot_z.y + p_2_n_x_x*connect_agent_2_dot_z.x + p_2_n_x_y*connect_agent_2_dot_z.y; 

	    dot_c_v_y = (p_1_s_y_x + p_2_s_y_x)*agent_0_dot_z.x + (p_1_s_y_y + p_2_s_y_y)*agent_0_dot_z.y + p_1_n_y_x*connect_agent_1_dot_z.x + p_1_n_y_y*connect_agent_1_dot_z.y + p_2_n_y_x*connect_agent_2_dot_z.x + p_2_n_y_y*connect_agent_2_dot_z.y;   		
	
	}
	else if (V1_vertice_counter == 5)
	{
	    ...
	    p_3_s_x_x = partial_derivative_centroid_over_self_state_x_x(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y);  

	    p_1_s_y_x = partial_derivative_centroid_over_self_state_y_x(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_s_y_x = partial_derivative_centroid_over_self_state_y_x(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  

	    p_3_s_y_x = partial_derivative_centroid_over_self_state_y_x(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y);  	

	    p_1_s_x_y = partial_derivative_centroid_over_self_state_x_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_s_x_y = partial_derivative_centroid_over_self_state_x_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_3_s_x_y = partial_derivative_centroid_over_self_state_x_y(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_1_s_y_y = partial_derivative_centroid_over_self_state_y_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_s_y_y = partial_derivative_centroid_over_self_state_y_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  	

	    p_3_s_y_y = partial_derivative_centroid_over_self_state_y_y(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y); 

	    p_1_n_x_x = partial_derivative_centroid_over_self_state_x_x(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_x_x = partial_derivative_centroid_over_self_state_x_x(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y); 

	    p_3_n_x_x = partial_derivative_centroid_over_self_state_x_x(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y);  

	    p_1_n_y_x = partial_derivative_centroid_over_self_state_y_x(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_y_x = partial_derivative_centroid_over_self_state_y_x(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  

	    p_3_n_y_x = partial_derivative_centroid_over_self_state_y_x(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y);  	

	    p_1_n_x_y = partial_derivative_centroid_over_self_state_x_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_x_y = partial_derivative_centroid_over_self_state_x_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_3_n_x_y = partial_derivative_centroid_over_self_state_x_y(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_1_n_y_y = partial_derivative_centroid_over_self_state_y_y(boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    p_2_n_y_y = partial_derivative_centroid_over_self_state_y_y(boundary2_vertices[0], boundary2_vertices[1], boundary2_vertices[2], boundary2_vertices[3], connection_2.x1, connection_2.y1, connection_2.x2, connection_2.y2, centroid_1.m, centroid_1.x, centroid_1.y);  	

	    p_3_n_y_y = partial_derivative_centroid_over_self_state_y_y(boundary3_vertices[0], boundary3_vertices[1], boundary3_vertices[2], boundary3_vertices[3], connection_3.x1, connection_3.y1, connection_3.x2, connection_3.y2, centroid_1.m, centroid_1.x, centroid_1.y);

	    dot_c_v_x = (p_1_s_x_x + p_2_s_x_x + p_3_s_x_x)*agent_0_dot_z.x + (p_1_s_x_y + p_2_s_x_y + p_3_s_x_y)*agent_0_dot_z.y + p_1_n_x_x*connect_agent_1_dot_z.x + p_1_n_x_y*connect_agent_1_dot_z.y + p_2_n_x_x*connect_agent_2_dot_z.x + p_2_n_x_y*connect_agent_2_dot_z.y + p_3_n_x_x*connect_agent_3_dot_z.x + p_3_n_x_y*connect_agent_3_dot_z.y; 

	    dot_c_v_y = (p_1_s_y_x + p_2_s_y_x + p_3_s_y_x)*agent_0_dot_z.x + (p_1_s_y_y + p_2_s_y_y + p_3_s_y_y)*agent_0_dot_z.y + p_1_n_y_x*connect_agent_1_dot_z.x + p_1_n_y_y*connect_agent_1_dot_z.y + p_2_n_y_x*connect_agent_2_dot_z.x + p_2_n_y_y*connect_agent_2_dot_z.y + p_3_n_y_x*connect_agent_3_dot_z.x + p_3_n_y_y*connect_agent_3_dot_z.y;  			
...
/*
if(V1_vertice_counter == 4)
{
ROS_INFO("p_1: %ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld", boundary1_vertices[0], boundary1_vertices[1], boundary1_vertices[2], boundary1_vertices[3], connection_1.x1, connection_1.y1, connection_1.x2, connection_1.y2, centroid_1.m, centroid_1.x, centroid_1.y);
}
*/	
...
*/   
    ...
    double temp_c, temp_v;
    temp_v = ((z_x - (double)centroid_1.x)*162*cos(alpha+1.5707963) + (z_y - (double)centroid_1.y)*162*sin(alpha+1.5707963))/3.1415926*180;
    ...
    vt_ctr.angular.x = -0.00001*162*temp_v*cos(alpha+1.5707963);
    vt_ctr.angular.y = -0.00001*162*temp_v*sin(alpha+1.5707963);

    double dot_u = 0, threshold = 0;

    if(time_t >= trigger_time + trigger_interval)
    {

	trigger_time = time_t;
	control_update = 1;
	event.z = time_t;//trigger_interval;//time_t;		
    }
    
    if (control_update == 1)
    {
				
    	dot_u = 0.00001*omega_0*( ( (vt_ctr.angular.x - dot_c_v_x)*162*cos(alpha+1.5707963) + (vt_ctr.angular.y - dot_c_v_y)*162*sin(alpha+1.5707963) )/3.1415926*180 + ( (z_x - (double)centroid_1.x)*162*sin(alpha+1.5707963) + (z_y - (double)centroid_1.y)*162*cos(alpha+1.5707963) )/3.1415926*180 );   
    	...
    	trigger_interval = threshold/abs(0.2*dot_u);
    ...
    //ROS_INFO("time_t:%f", time_t);

    //ROS_INFO("trigger_interval:%f", trigger_interval);
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pub  = push.advertise<geometry_msgs::Twist>("/vehicle/vt_ctr",1000);
    ...
    */ 
    ...
    while(ros::ok()) 
    {
	if(!( (vt_ctr.linear.x==0)&&(vt_ctr.linear.y==0)&&(vt_ctr.linear.z==0) && (vt_ctr.angular.x==0)&&(vt_ctr.angular.y==0)&&(vt_ctr.angular.z==0) ))
	{
		pub.publish(vt_ctr);
	}
