...
// Custom transferred packet by users
tip::UnicycleInfoMsg transferedData;
...
std_msgs::Float64 n1_dVBLF;
std_msgs::Float64 n2_dVBLF;
std_msgs::Float64 n3_dVBLF;
...
//std_msgs::Float32 deltaVBLF1;
double deltaVBLF0;	// Own VBLF
double deltaVBLF1;
double deltaVBLF2;
double deltaVBLF3;
...
void MyTest(const tip::UnicycleInfoMsg tmp)
{
    ;
}
...
void N1VBLFCb(const std_msgs::Float64& n1_dVBLF)
{
    deltaVBLF1 = n1_dVBLF.data;
}
...
void N2VBLFCb(const std_msgs::Float64& n2_dVBLF)
{
    deltaVBLF2 = n2_dVBLF.data;
}
...
void N3VBLFCb(const std_msgs::Float64& n3_dVBLF)
{
    deltaVBLF3 = n3_dVBLF.data;
}
...
// This method computes the BLF cost function of each agent
double computeBLF(Point zk, Point ck, Point* A, double* b, int nWorldVertexes){
	double retV = 0; 
	for (int i = 0; i < nWorldVertexes; i++){
		retV += 1 / (b[i] - zk.dot(A[i]));
	}
	Point tmp = zk - ck;
	retV *= tmp.dot(tmp) / 2; 
	return retV;
...
/*************************************************************************compute virtual center*****************************************************************************/
    ...
    z_x = center.position.x*1000 - 162/omega_0*sin(alpha+1.5707963);
    z_y = center.position.y*1000 + 162/omega_0*cos(alpha+1.5707963);
    v_0 = 16;
...
*/   
//#define USING_SWITCHING_CONTROL
#ifdef USING_SWITCHING_CONTROL
    double temp_c;
    ...
    event.y = threshold;
#else 
    static Point lastCentroid0, lastCentroid1, lastCentroid2, lastCentroid3;
    static Point lastVM0, lastVM1, lastVM2, lastVM3;
    static Point lastV0, lastV1, lastV2, lastV3;
    static double myLastBLF = 0;
    static int nLines = 4;
    static Point ALines[4] = {{0 , -1}, {0 , 1}, {-1 , 0}, {1 , 0}};
    static double bLines[4] = {-20 , 2800 , -20, 4000};
	printf("Start ----------------------------\n");
	printf("Me: %f %f %f \n", z_x, z_y, alpha);
	printf("NB: 1 %f %f| 2 %f %f | 3 %f %f \n", agent_1.x, agent_1.y, agent_2.x, agent_2.y, agent_3.x, agent_3.y);


    // Compute the actual BLF cost function and published to the neighbor agents
    double myBLF = computeBLF(agent_0, centroid_1, ALines, bLines, nLines); 

    // Compute the diferrence of BLF (V_k)
    // This value will be updated in a global scale and published through ROS service
    deltaVBLF0 =  myBLF - myLastBLF;

    // Compute the difference of virtual center to compute dV/dz (numerical euler derivative)
    Point dz0 = agent_0 - lastVM0;
    Point dz1 = agent_1 - lastVM1;
    Point dz2 = agent_2 - lastVM2;
    Point dz3 = agent_3 - lastVM3;

    Vector2 dVdz0, dVdz1, dVdz2, dVdz3;
    if(dz0.x != 0 && dz0.y != 0)
    	dVdz0 = dVdz0.getGradient2(deltaVBLF0, dz0);
    if(dz1.x != 0 && dz1.y != 0)
    	dVdz1 = dVdz1.getGradient2(deltaVBLF1, dz1);
    if(dz2.x != 0 && dz2.y != 0)
	    dVdz2 = dVdz2.getGradient2(deltaVBLF2, dz2);
    if(dz3.x != 0 && dz3.y != 0)
    	dVdz3 = dVdz3.getGradient2(deltaVBLF3, dz3);
	
	printf("dz 0=%f %f /t 1=%f %f /t 2=%f %f/t 3=%f %f\n", dz0.x, dz0.y, dz1.x, dz1.y, dz2.x, dz2.y, dz3.x, dz3.y);
	printf("dV   0=%f %/t 1=%f /t 2=%f /t 3=%f \n", deltaVBLF0, deltaVBLF1, deltaVBLF2, deltaVBLF3);
	printf("dVdz 0=%f %f /t 1=%f %f /t 2=%f %f/t 3=%f %f\n", dVdz0.x, dVdz0.y, dVdz1.x, dVdz1.y, dVdz2.x, dVdz2.y, dVdz3.x, dVdz3.y);

    // Determine the control input if the neighbor agents are known
    // Accumulate all the dVdz_i of it self and all adjacent agents
    // Assume now every agents are adjacent
    Vector2 sumdV = dVdz0;
    sumdV = sumdV + dVdz1 + dVdz2 + dVdz3;   // This line: assumption all agents are adjacent
	printf("sum dVdz: %f %f\n", sumdV.x, sumdV.y);

    /* Smooth Control input */
    static const double gain = 2;

    double tmpInput = sumdV.x * cos(alpha) + sumdV.y * sin(alpha);
	double temp_c = omega_0 + gain * sgn(omega_0) * sgn(tmpInput);
	
	printf("Input: %f\n", temp_c);
	printf("End ----------------------------\n");


    // Update variables for the computation of the next step
    lastVM0 = agent_0;
    lastVM1 = agent_1; 
    lastVM2 = agent_2;
    lastVM3 = agent_3;
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pubBLF = BLF.advertise<std_msgs::Float64>("/vehicle/deltaVBLF",1000);
    ros::Publisher pube = vect.advertise<geometry_msgs::Vector3>("/vehicle/event",1000);
    //ros::Publisher pTest = BLF.advertise<UnicycleInfoMsg::UnicycleInfoStruct>("/vehicle/test",1000);
    ...
    // Subscribe to the BLF of the neighbor agents
    ros::Subscriber neib1Vtopic = neighbour1.subscribe("/jet1/deltaVBLF", 1000, N1VBLFCb);
    ros::Subscriber neib2Vtopic = neighbour2.subscribe("/jet2/deltaVBLF", 1000, N2VBLFCb);
    ros::Subscriber neib3Vtopic = neighbour3.subscribe("/jet3/deltaVBLF", 1000, N3VBLFCb);
